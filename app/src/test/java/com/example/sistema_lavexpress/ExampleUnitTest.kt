package com.example.sistema_lavexpress
...
suspend fun registrarEntrada(maquina: Maquina, slots: Array<Slot>): EstadoOperacion {
    ...
    return EstadoOperacion.Exito("Máquina ${maquina.codigo} asignada al Slot #${slotDisponible.numero}")
}
...
suspend fun registrarSalida(
    ...
    } ?: return EstadoOperacion.Error("Máquina con código $codigoBuscar no encontrada en el sistema.")
...
class LavExpressPrueba {
    ...
    fun main(): Unit = runBlocking {
        ...
        println("--- PRUEBA DE ERRORES DE CÓDIGO (EXCEPCIONES) ---")
        try {
        ...
        println("--- REGISTRO DE ENTRADAS ASÍNCRONAS ---")
        ...
        for (m in maquinasIngresar) {
            ...
            when (resEntrada) {
                ...
                is EstadoOperacion.Exito -> println("ÉXITO: ${resEntrada.mensaje}")
        ...
        println("--- ANÁLISIS DE DATOS Y REPORTE DE CIERRE ---")
        ...
        println("Cantidad de máquinas atendidas de usuarios Suscriptores: $cantidadSuscriptores")

        val ingresoPromedio =
            if (historialTickets.isNotEmpty()) totalRecaudado / historialTickets.size else 0.0
        println("Ingreso promedio por máquina: $${"%.0f".format(ingresoPromedio)}")

        val codigosFinalizados = historialTickets.map { it.maquina.codigo }
        println("Códigos de máquinas que finalizaron turno: $codigosFinalizados")

        val maquinaMaxUso = historialTickets.maxByOrNull { it.minutosUso }
        println("Máquina con más tiempo de uso: ${maquinaMaxUso?.maquina?.codigo} con ${maquinaMaxUso?.minutosUso} minutos")
